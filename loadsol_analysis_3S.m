...
%% Code Adjustments
% Expanded on by Nicole Stark (nestark@vt.edu) at Virginia Tech 
% The expansion was made to intergrate the three sensor loadsol 
% Currently the expansion only incorperates the full sensor reading

% 5/24/21
% The code was expanded again to include GRF curves, 1st peak (p-GRF), and 2nd
% peak (WA-GRF)

%6/25/21
%additional export for stance time was added
...
function LoadData_Callback(hObject, eventdata, handles)
    ...
    open_LS = importdata(tempstr,' ',10);
    data = [];
    data(:,1) = open_LS.data(:,1);
    data(:,2) = open_LS.data(:,5);
    data(:,3) = open_LS.data(:,10);
    ...
    temp0(1:20) = ' ';
    temp0(21:28) = temp(81:88);
    temp0(29:60) = ' ';
    temp0(61:68) = temp(181:188); 
    temp0(69:100) = ' ';
    temp = temp0;
...
%handles.Movement.String{5} = 'Sit to Stand';
guidata(hObject,handles);    
plot_data(hObject, eventdata, handles) 
    ...
    for t = 1:handles.nTrials
        ...
        if isempty(handles.closedtrials) || ~any(handles.closedtrials == t)
            ...
            for Step3 = 1:length(IC_events_3)
                ...
                % changed to look from IC to PPF length (was an error that
                % it looked up to PIF)
                [peaks2,locs2] = findpeaks(data((IC_events_3(Step3)+PPFLength):TO_events_3(Step3),3));
...
%                 [peaks2,locs2] = findpeaks(data((IC_events_2(Step2)+PPFLength):TO_events_2(Step2),2));
    ...
    eval(['T = table(Step,PIF_' handles.name2(end) ',PIF_' handles.name3(end) ',PIF_Symm,' ...
        'PPF_' handles.name2(end) ',PPF_' handles.name3(end) ',PPF_Symm,'...
        'ILR_' handles.name2(end) ',ILR_' handles.name3(end) ',ILR_Symm,'...
        'ALR_' handles.name2(end) ',ALR_' handles.name3(end) ',ALR_Symm,'...
