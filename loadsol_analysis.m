...
elseif MovementType == 6 %Triple Hop
PIFLength =  floor(TimeCut_PIF*.001*handles.SamplingFreq);
IMPLength =  floor(TimeCut_IMP*.001*handles.SamplingFreq);    
c2 = 1;
cur2=1;
...
if isempty(handles.closedtrials) || ~any(handles.closedtrials == t)
    ...
    eval(['TO_events_3 = handles.T' num2str(t) '_TO_3;'])
                ...
                if length(IC_events_2)>length(IC_events_3) % Analyze 2, 2=Left Leg, 3=Right Leg

                [peaks,locs] = findpeaks(data(IC_events_2(1):IC_events_2(1)+PIFLength,2)); %first landing
                [peaks2,locs2] = findpeaks(data(IC_events_2(2):IC_events_2(2)+PIFLength,2)); %second landing
                [peaks3,locs3] = findpeaks(data(IC_events_2(3):IC_events_2(3)+PIFLength,2)); %third landing
                
                [peaks_ppf,locs_ppf] = findpeaks(data(TO_events_2(1)-PIFLength:TO_events_2(1),2)); % first pushoff
                [peaks_ppf2,locs_ppf2] = findpeaks(data(TO_events_2(2)-PIFLength:TO_events_2(2),2)); %pushoff on first landing
                [peaks_ppf3,locs_ppf3] = findpeaks(data(TO_events_2(3)-PIFLength:TO_events_2(3),2)); %pushoff on second landing
                
                [V_ppf, I_ppf]=sort(peaks_ppf, 'descend');
                PPF_2(c2,1) = TO_events_2(1)-PIFLength+locs(I_ppf(1));
                PPF_2(c2,2) = V_ppf(1);
                ...
                [V_ppf2, I_ppf2] = sort(peaks_ppf2, 'descend');
                PPF_2(c2,3) = TO_events_2(2)-PIFLength+locs(I_ppf2(1));
                PPF_2(c2,4) = V_ppf2(1);
                
                %second landing
                 if isempty(locs2) %Then just take max
                    PIF_2(c2,3) = IC_events_2(2)+PIFLength;
                    PIF_2(c2,4) = data(IC_events_2(2)+PIFLength,2);
                    LR_2(c2,2) = PIF_2(c3,4)/PIFLength*handles.SamplingFreq;
                else
                    [V2,I2] = sort(peaks2,'descend');
                    PIF_2(c2,3) = IC_events_2(2)+locs2(I2(1));
                    PIF_2(c2,4) = V2(1);
                    LR_2(c2,2) = PIF_2(c2,4)/locs2(I2(1))*handles.SamplingFreq;
                 end
                
                
                [V_ppf3, I_ppf3]=sort(peaks_ppf3, 'descend');
                PPF_2(c2,5) = TO_events_2(3)-PIFLength+locs(I_ppf3(1));
                PPF_2(c2,6) = V_ppf3(1);
                
                 %third landing 
                 if isempty(locs3) %Then just take max
                    PIF_2(c2,5) = IC_events_2(3)+PIFLength;
                    PIF_2(c2,6) = data(IC_events_2(3)+PIFLength,2);
                    LR_2(c2,3) = PIF_2(c2,6)/PIFLength*handles.SamplingFreq;
                else
                    [V3,I3] = sort(peaks3,'descend');
                    PIF_2(c2,5) = IC_events_2(1)+locs3(I3(1));
                    PIF_2(c2,6) = V3(1);
                    LR_2(c2,3) = PIF_2(c2,6)/locs3(I3(1))*handles.SamplingFreq;
                 end               
                 
                %first landing
                time_TO_2(c2,1)=data(TO_events_2(1),1);
                time_IC_2(c2,1)=data(IC_events_2(1),1);
                %time_PIF_2(c2)=data(PIF_2(1),1);
                %second landing
                time_TO_2(c2,2)=data(TO_events_2(2),1);
                time_IC_2(c2,2)=data(IC_events_2(2),1);
                %third landing
                time_TO_2(c2,3)=data(TO_events_2(3),1);
                time_IC_2(c2,3)=data(IC_events_2(3),1);
                
                
                %first landing            
                IMP_2(c2,1) = trapz(data(IC_events_2(1):IC_events_2(1)+IMPLength,1),...
                    data(IC_events_2(1):IC_events_2(1)+IMPLength,2));
                %second landing
                IMP_2(c2,2) = trapz(data(IC_events_2(2):IC_events_2(2)+IMPLength,1),...
                    data(IC_events_2(2):IC_events_2(2)+IMPLength,2));
                %third landing
                IMP_2(c2,3) = trapz(data(IC_events_2(3):IC_events_2(3)+IMPLength,1),...
                    data(IC_events_2(3):IC_events_2(3)+IMPLength,2));
                
                
                Curve2(:,c2) = data(IC_events_2(1):IC_events_2(1)+PIFLength,2);
                Curve2(:,cur2+1) = data(IC_events_2(2):IC_events_2(2)+PIFLength,2);
                Curve2(:,cur2+2) = data(IC_events_2(3):IC_events_2(3)+PIFLength,2);
                ...
                else % everything below is analysis on right leg (above is on left)
                    
                    
                [peaks,locs] = findpeaks(data(IC_events_3(1):IC_events_3(1)+PIFLength,3)); %first landing
                [peaks2,locs2] = findpeaks(data(IC_events_3(2):IC_events_3(2)+PIFLength,3)); %second landing
                [peaks3,locs3] = findpeaks(data(IC_events_3(3):IC_events_3(3)+PIFLength,3)); %third landing
                
                [peaks_ppf,locs_ppf] = findpeaks(data(TO_events_3(1)-PIFLength:TO_events_3(1),3)); % first pushoff
                [peaks_ppf2,locs_ppf2] = findpeaks(data(TO_events_3(2)-PIFLength:TO_events_3(2),3)); %pushoff on first landing
                [peaks_ppf3,locs_ppf3] = findpeaks(data(TO_events_3(3)-PIFLength:TO_events_3(3),3)); %pushoff on second landing
                
                [V_ppf, I_ppf]=sort(peaks_ppf, 'descend');
                PPF_3(c3,1) = TO_events_3(1)-PIFLength+locs(I_ppf(1));
                PPF_3(c3,2) = V_ppf(1);
                ...
                if isempty(locs) %Then just take max
                    PIF_3(c3,1) = IC_events_3(1)+PIFLength;
                    PIF_3(c3,2) = data(IC_events_3(1)+PIFLength,3);
                    LR_3(c3,1) = PIF_3(c3,2)/PIFLength*handles.SamplingFreq;
                else
                    [V,I] = sort(peaks,'descend');
                    PIF_3(c3,1) = IC_events_3(1)+locs(I(1));
                    PIF_3(c3,2) = V(1);
                    LR_3(c3,1) = PIF_3(c3,2)/locs(I(1))*handles.SamplingFreq;
                end
           
                [V_ppf2, I_ppf2]=sort(peaks_ppf2, 'descend');
                PPF_3(c3,3) = TO_events_3(2)-PIFLength+locs(I_ppf2(1));
                PPF_3(c3,4) = V_ppf2(1);
                
                %second landing
                 if isempty(locs2) %Then just take max
                    PIF_3(c3,3) = IC_events_3(2)+PIFLength;
                    PIF_3(c3,4) = data(IC_events_3(2)+PIFLength,3);
                    LR_3(c3,2) = PIF_3(c3,4)/PIFLength*handles.SamplingFreq;
                else
                    [V2,I2] = sort(peaks2,'descend');
                    PIF_3(c3,3) = IC_events_3(2)+locs2(I2(1));
                    PIF_3(c3,4) = V2(1);
                    LR_3(c3,2) = PIF_3(c3,4)/locs2(I2(1))*handles.SamplingFreq;
                 end
                 
                [V_ppf3, I_ppf3]=sort(peaks_ppf3, 'descend');
                PPF_3(c3,5) = TO_events_3(3)-PIFLength+locs(I_ppf3(1));
                PPF_3(c3,6) = V_ppf3(1);
                
                 %third landing 
                 if isempty(locs3) %Then just take max
                    PIF_3(c3,5) = IC_events_3(3)+PIFLength;
                    PIF_3(c3,6) = data(IC_events_3(3)+PIFLength,3);
                    LR_3(c3,3) = PIF_3(c3,6)/PIFLength*handles.SamplingFreq;
                else
                    [V3,I3] = sort(peaks3,'descend');
                    PIF_3(c3,5) = IC_events_3(1)+locs3(I3(1));
                    PIF_3(c3,6) = V3(1);
                    LR_3(c3,3) = PIF_3(c3,6)/locs3(I3(1))*handles.SamplingFreq;
                 end
                
                %first landing
                time_TO_3(c3,1)=data(TO_events_3(1),1);
                time_IC_3(c3,1)=data(IC_events_3(1),1);
                %time_PIF_3(c3)=data(PIF_3(1),1);
                %second landing
                time_TO_3(c3,2)=data(TO_events_3(2),1);
                time_IC_3(c3,2)=data(IC_events_3(2),1);
                %third landing
                time_TO_3(c3,3)=data(TO_events_3(3),1);
                time_IC_3(c3,3)=data(IC_events_3(3),1);
                
                %first landing
                IMP_3(c3,1) = trapz(data(IC_events_3(1):IC_events_3(1)+IMPLength,1),...
                    data(IC_events_3(1):IC_events_3(1)+IMPLength,3));
                %second landing
                IMP_3(c3,2) = trapz(data(IC_events_3(2):IC_events_3(2)+IMPLength,1),...
                    data(IC_events_3(2):IC_events_3(2)+IMPLength,3));
                %third landing
                IMP_3(c3,3) = trapz(data(IC_events_3(3):IC_events_3(3)+IMPLength,1),...
                    data(IC_events_3(3):IC_events_3(3)+IMPLength,3));

                
                %need to add for second and third landings
                
                Curve3(:,cur3) = data(IC_events_3(1):IC_events_3(1)+PIFLength,3);
                Curve3(:,cur3+1) = data(IC_events_3(2):IC_events_3(2)+PIFLength,3);
                Curve3(:,cur3+2) = data(IC_events_3(3):IC_events_3(3)+PIFLength,3);
    ...
    PIF_2=[PIF_2_1,PIF_2_2, PIF_2_3];
    PIF_3=[PIF_3_1,PIF_3_2, PIF_3_3];   
    
    if length(PIF_2_1)>length(PIF_3_1)
        trim2 = length(PIF_2_1)-length(PIF_3_1);
        trim3 = 0;
    else
        trim2 = length(PIF_2_1)-length(PIF_3_1);
        trim3 = 0;        
...
%         
%         if get(handles.DominantLimb,'Value') == 1
%         PPF_Symm1 = (mean(PPF_3(1:end-trim3,1))./mean(PPF_2(1:end-trim2,1)))*100;  
%         PIF_Symm1 = (mean(PIF_3(1:end-trim3,1))./mean(PIF_2(1:end-trim2,1)))*100;
%         LR_Symm1 = (mean(LR_3(1:end-trim3,1))./mean(LR_2(1:end-trim2,1)))*100;
%         IMP_Symm1 = (mean(IMP_3(1:end-trim3,1))./mean(IMP_2(1:end-trim2,1)))*100;
%         else
%         PPF_Symm1 = (mean(PPF_2(1:end-trim2))./mean(PPF_3(1:end-trim3)))*100;
%         PIF_Symm1 = (mean(PIF_2(1:end-trim2))./mean(PIF_3(1:end-trim3)))*100;
%         LR_Symm1 = (mean(LR_2(1:end-trim2))./mean(LR_3(1:end-trim3)))*100;
%         IMP_Symm1 = (mean(IMP_2(1:end-trim2))./mean(IMP_3(1:end-trim3)))*100;
%         
%         
%         end
%         
...
%     PPF_2 = [PPF_2(1:end-trim2);mean(PPF_2(1:end-trim2));std(PIPF_2(1:end-trim2))];
...
%         'IMP_' handles.name2(end) ',IMP_' handles.name3(end) ',time_TO_' handles.name2(end) ',time_TO_' handles.name3(end) ',time_IC_' handles.name2(end) ',time_IC_' handles.name3(end) ')'])
%     
    eval(['T = table(PPF_' handles.name2(end) ',PPF_' handles.name3(end) ',PIF_' handles.name2(end) ',PIF_' handles.name3(end)...
        ',LR_' handles.name2(end) ',LR_' handles.name3(end)...
        ',IMP_' handles.name2(end) ',IMP_' handles.name3(end) ',time_TO_' handles.name2(end) ',time_TO_' handles.name3(end) ',time_IC_' handles.name2(end) ',time_IC_' handles.name3(end) ')'])
